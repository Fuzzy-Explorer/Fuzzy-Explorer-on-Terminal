...
do
  ...
  # fzfでのディレクトリの選択
  _fet_path_selected_path=$(echo -e "$_fet_path_path_list" | fzf +m --ansi --height 70% --cycle --preview-window right:40% --info='inline' --layout=reverse --border --prompt="$_fet_var_promp >" --header="$_fet_var_infobar" --bind "$_fet_var_keybindings" --preview="echo {} | cut -f 2 -d ' ' | xargs -rI{a} sh -c 'if [ -f \"{a}\" ]; then ls -ldhG {a}; if [ -n \"$(which batcat)\" ]; then batcat {a} --color=always --style=grid --line-range :100; else cat {a}; fi else ls -ldhG {a}; echo; lsi {a}; fi'")
